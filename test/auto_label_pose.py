...
INPUT_DIR = BASE_DIR / "pose_images"
OUTPUT_DIR = BASE_DIR / "pose_autolabel"
...
def make_label_line(result, person_index: int) -> str:
    """Convert one predicted person to an Ultralytics YOLO Pose label line."""
    box = result.boxes.xywhn[person_index].detach().cpu().tolist()
    cls_id = int(result.boxes.cls[person_index].item())
    ...
    for (x, y), confidence in zip(keypoints_xy, keypoints_conf):
        if confidence >= KEYPOINT_CONF:
            # Draft auto-label: predicted keypoint is treated as visible.
            values.extend((f"{x:.6f}", f"{y:.6f}", "2"))
        else:
            # Missing/uncertain keypoint.
    ...
    for index, image_path in enumerate(image_paths, start=1):
        ...
        if (
            ...
            for person_index in range(person_count):
                lines.append(make_label_line(result, person_index))
