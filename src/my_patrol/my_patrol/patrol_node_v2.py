...
class MarkerListener(Node):
    ...
    CHARGER_CENTER_TOL = 0.05  # 충전소 마커 중앙 정렬 허용 오차
    CHARGER_TURN_SPEED = 0.25  # 정렬 후 180도 회전속도(rad/s)
    ...
    def __init__(self):
        ...
        self.create_subscription(
            Int32MultiArray, '/charger_marker', self._charger_id_cb, 10)
        self.create_subscription(
            Float32, '/charger_offset', self._charger_offset_cb, 10)
    ...
    def _charger_id_cb(self, msg):
        self.charger_visible = 249 in msg.data

    def _charger_offset_cb(self, msg):
        self.latest_charger_offset = msg.data
        self.charger_offset_time = time.time()
    ...
    def _charger_marker_visible(self):
        return (
            self.charger_visible
            and self.charger_offset_time is not None
            and time.time() - self.charger_offset_time < self.SEEN_TIMEOUT
        )

    def align_to_charger(self, timeout=10.0):
        #충전소 마커에 정렬한 뒤 제자리에서 180도 회전
        self.charger_visible = False
        self.charger_offset_time = None
        ...
        self.get_logger().info('충전소 마커 정렬 대기')

        # 1. ID 249가 보일 때만 화면 중앙으로 미세 정렬
        while time.time() < end and rclpy.ok():
            rclpy.spin_once(self, timeout_sec=0.05)
            tw = Twist()

            if not self._charger_marker_visible():
                # 지정 좌표에서 마커가 안 보이면 회전하지 않고 정지한다.
                self.cmd_pub.publish(tw)
                if not waiting_logged:
                    self.get_logger().info('ID 249 마커를 기다리는 중 ...')
                    waiting_logged = True
                continue
            else:
                waiting_logged = False
                ...
                angular = -self.ALIGN_K * offset
                magnitude = min(
                    self.MAX_TURN,
                    max(self.MIN_TURN, abs(angular)),
                )
                tw.angular.z = magnitude if angular > 0 else -magnitude
        ...
        if self.yaw is None:
            self.get_logger().warn('odom이 없어 180도 회전할 수 없습니다')
            return False

        self.get_logger().info('마커 중앙 정렬 완료, 180도 회전 시작')
        target = math.pi
        rotated = 0.0
        previous_yaw = self.yaw
        turn_end = time.time() + self.CHARGER_TURN_TIMEOUT

        while (rotated < target and time.time() < turn_end and
               rclpy.ok()):
            rclpy.spin_once(self, timeout_sec=0.05)
            if self.yaw is not None:
                delta = self.yaw - previous_yaw
            ...
            tw = Twist()
            tw.angular.z = self.CHARGER_TURN_SPEED
            self.cmd_pub.publish(tw)

        self.cmd_pub.publish(Twist())

        if rotated < target:
            self.get_logger().warn('충전 방향 180도 회전 실패')
            return False

        self.get_logger().info('충전중 ...')
        return True
